...
params$parser <- list(iterations = 30, 
...
As you have seen above in the example, if you want to train also the dependency parser, you also need pre-trained word embeddings which you can provide in the `embedding_form_file` argument. Example training data can be found at https://lindat.mff.cuni.cz/repository/xmlui/handle/11234/1-2364. If you also have a holdout file in CONLL-U format, the training is stopped before model performance decreases on the holdout CONLL-U file.
