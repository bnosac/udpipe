...
If you have this type of file and you provide it to `udpipe_train`, a model is saved on disk in a binary format which can then be used to annotate your text data using `udpipe_annotate` in order to Tokenize, get Parts of Speech tags, Lemma's or Dependency relationships.
...
## The model is now trained and saved in file toyexample.udpipe in the current working directory
...
x_tagged <- udpipe_annotate(object = mymodel, x = txt, tagger = "none", parser = "none")
head(as.data.frame(x_tagged), 2)
```

In the above example, we trained only a tokenizer. This was done by setting the `annotation_tagger` and the `annotation_parser` argument to 'none'. If you want to create a tagger which is capable of executing tokenisation, tagging as well as dependency parsing with the default settings of the algoritm, you just proceed as follows.
...
When you want to train the model with specific tokenizer/tagger/parser settings, you need to provide these settings as a character string to the arguments `annotation_tokenizer`, `annotation_tagger` and `annotation_parser`. This should be a comma-separated character string of arguments with their values. As shown below.

The possible options for each of these settings are explained in detail below and their logic is detailed in the paper "Tokenizing, POS Tagging, Lemmatizing and Parsing UD 2.0 with UDPipe", available at <http://ufal.mff.cuni.cz/~straka/papers/2017-conll_udpipe.pdf>.

```{r, eval=FALSE}
params <- list()

## Tokenizer training parameters
params$tokenizer <- "dimension=24;epochs=100;initialization_range=0.1;batch_size=100;learning_rate=0.005;dropout=0.1;early_stopping=1"

## Tagger training parameters
params$tagger <- "models=2;templates_1=tagger;guesser_suffix_rules_1=8;guesser_enrich_dictionary_1=6;guesser_prefixes_max_1=0;use_lemma_1=0;use_xpostag_1=1;use_feats_1=1;provide_lemma_1=0;provide_xpostag_1=1;provide_feats_1=1;prune_features_1=0;templates_2=lemmatizer;guesser_suffix_rules_2=6;guesser_enrich_dictionary_2=4;guesser_prefixes_max_2=4;use_lemma_2=1;use_xpostag_2=0;use_feats_2=0;provide_lemma_2=1;provide_xpostag_2=0;provide_feats_2=0;prune_features_2=0"

## Dependency parser training parameters
params$parser <- "iterations=30;embedding_upostag=20;embedding_feats=20;embedding_xpostag=0;embedding_form=50;embedding_form_file=../ud-2.0-embeddings/nl.skip.forms.50.vectors;embedding_lemma=0;embedding_deprel=20;learning_rate=0.01;learning_rate_final=0.001;l2=0.5;hidden_layer=200;batch_size=10;transition_system=projective;transition_oracle=dynamic;structured_interval=10"

## Train the model
m <- udpipe_train(file = "toyexample.udpipe", 
                  files_conllu_training = "traindata.conllu", files_conllu_holdout = "holdout.conllu", 
                  annotation_tokenizer = params$tokenizer,
                  annotation_tagger = params$tagger,
                  annotation_parser = params$parser)
```

As you have seen above in the example, if you want to train also the dependency parser, you also need pre-trained word embeddings. Example training data can be found at https://lindat.mff.cuni.cz/repository/xmlui/handle/11234/1-2364.

Mark that `udpipe_train` also allows you to give these annotation arguments as a list so for the above example, this would be:

```{r, eval=FALSE}
params$tokenizer <- list(dimension = 24, epochs = 100, initialization_range = 0.1, 
    batch_size = 100, learning_rate = 0.005, dropout = 0.1, early_stopping = 1)
```

Before you embark in starting to train your own models, consider that training can take a while. In order to progress the evolution, you can set the environment variable `UDPIPE_PROCESS_LOG` to a location of a file on disk. The evolution of the training will be put in that log so that you can see where the algorithm is. Mark that you need to do this before you initially load the udpipe package as shown below, otherwise the script will not pick up this and will not write progress to the log.
...
```{r}
x <- readLines(system.file(package = "udpipe", "models-ud-2.0", "params_tokenizer"))
cat(head(x, n = 5), sep = "\n")
```
...
```{r}
x <- readLines(system.file(package = "udpipe", "models-ud-2.0", "params_tagger"))
cat(head(x, n = 5), sep = "\n")
```
...
```{r}
x <- readLines(system.file(package = "udpipe", "models-ud-2.0", "params_parser"))
cat(head(x, n = 5), sep = "\n")
```
